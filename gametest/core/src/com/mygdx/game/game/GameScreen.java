...
public class GameScreen extends Screens {
    ...
    boolean setDispatch = false;
    boolean setDispatchMissil = false;
    Vector3 touchPoint;
    ...
    Table scoresBar;
    Label lbLevel, lbScore, lbLifeCount;
    ImageButton btPause;
    ...
    public GameScreen(final MyGdxGame game) {
        ...
        dialogPause = new Dialog(Assets.idioms.get("game_paused"), Assets.styleDialogPause);
        TextButton btContinue = new TextButton(Assets.idioms.get("continue"), Assets.styleTextButton);
        TextButton btMenu = new TextButton(Assets.idioms.get("main_menu"), Assets.styleTextButton);
        ...
        TextButton btTryAgain = new TextButton(Assets.idioms.get("try_again"), Assets.styleTextButton);
        TextButton btMenu2 = new TextButton(Assets.idioms.get("main_menu"), Assets.styleTextButton);
        ...
        lbLevel = new Label(Assets.idioms.get("level") + " " + oWorld.currentLevel, Assets.styleLabel);
        lbScore = new Label(Assets.idioms.get("score") + " " + oWorld.score, Assets.styleLabel);
        lbLifeCount = new Label("x" + oWorld.oSpaceShip.lives, Assets.styleLabel);
        Image movement = new Image(Assets.move);

        scoresBar = new Table();
        scoresBar.setBackground(Assets.InGameStatus);
        ...
        scoresBar.add(movement).size(20).right();
        scoresBar.add(lbLifeCount).right();
    ...
    private void updateRunning(float deltaTime) {
        ...
        if (Gdx.input.isKeyPressed(Keys.DPAD_LEFT) || Gdx.input.isKeyPressed(Keys.A) || Gdx.input.isKeyPressed(Keys.DPAD_RIGHT) || Gdx.input.isKeyPressed(Keys.D)) {
            ...
            oWorld.update(deltaTime, accel, setDispatch, setDispatchMissil);
            ...
            oWorld.update(deltaTime, Gdx.input.getAccelerometerX(), setDispatch, setDispatchMissil);
        } else {
            oWorld.update(deltaTime, accel, setDispatch, setDispatchMissil);
        }
        ...
        if (level != oWorld.currentLevel) {
            level = oWorld.currentLevel;
            lbLevel.setText(Assets.idioms.get("level") + " " + level);
        }

        lbScore.setText(Assets.idioms.get("score") + " " + oWorld.score);
        lbLifeCount.setText("x" + oWorld.oSpaceShip.lives);
    ...
    private void loadPreGame(float delta) {
        if (preGame == 0 && Settings.isTiltControl) {
            if (rotacion < -20 || rotacion > 20)
                addRotacion *= -1;
            rotacion += addRotacion;
            batcher.draw(Assets.help, SCREEN_WIDTH / 2f - 51, 190, 51, 0, 102, 200, 1, 1, rotacion);
        } else {
    ...
    private void presentReady() {
        String touchToStart = Assets.idioms.get("touch_to_start");
    ...
    @Override
    public void hide() {
        Settings.aggregateSpacing(oWorld.score);
        super.hide();
    }
