...
public class menuScreen extends InputAdapter implements Screen {
	...
	dinosaurs Yoshi = new dinosaurs("Yoshi",60.0f,10, "Flutter Kick");
	turtles Koopa = new turtles("Koopa",30.0f,10,"Troopa Dance");
	kings Bowser = new kings("Bowser",90.0f,10,"Grrrrrrrrr");
	...
	public void render(float delta) {
		...
		if (brick_rect.overlaps(dino_rect)) {
			System.out.println(Yoshi.getName()+" "+dkc.getCollision());
			player1y = prevdy;
			player1x = prevdx;
		}
		if (brick_rect.overlaps(turt_rect)) {
			System.out.println(Koopa.getName()+" "+tnc.getNonCollision());
		}
		if (brick_rect.overlaps(king_rect)) { 
			System.out.println(Bowser.getName()+" "+dkc.getCollision());
			player3y = prevky;
			player3x = prevkx;
		}	
